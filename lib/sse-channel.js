...
var encoder = require('eventsource-encoder');
 ...
 *
 * @param  {String|Object} message    Object or string to parse into sendable message
 * @param  {Boolean}       jsonEncode Whether to JSON-encode data parameter
 * @return {String}
 */
function parseMessage(message, jsonEncode) {
    const msg = typeof message === 'string' ? { data: message } : Object.assign({}, message);

    if (jsonEncode) {
        msg.data = JSON.stringify(typeof msg.data === 'undefined' ? '' : msg.data);
